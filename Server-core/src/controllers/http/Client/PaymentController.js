...
module.exports = {
  ...
  createRequestQRBank: async (req, res) => {
    try {
      const List = await BankListModel.findAll({
    where: {
        status: BankListModel.STATUS_ENUM.ACTIVE
      ...
      const { bank, amountDeposit } = req.body;

      if (!amountDeposit || Number(amountDeposit) < 50000) {
        return res.status(200).json({
          status: false,
          msg: "Tiền nạp tối thiểu phải là 50,000"
        });
      }

      let selectedBank = List.find(item => item.bankProvide === bank);

      if (!selectedBank) {
        return res.status(200).json({
          status: false,
          msg: "Vui lòng chọn bank khác"
        });
      }

      let baseData = '';
      let stk = selectedBank.bankNumber;

      if (bank === 'VIB') {
        baseData = `00020101021138510010A00000072701210108${stk}00069704160208QRIBFTTA53037045802VN`;
      } else if (bank === 'VIB') {
        baseData = `00020101021138540010A000000727012400069704360109${stk}08QRIBFTTA53037045802VN`;
      } else if (bank === 'TPBANK') {
        baseData = `00020101021138580010A000000727012800069704070111${stk}0208QRIBFTTA53037045802VN`;
      }

      const amount = Number(amountDeposit).toString();
      const amountTag = '54' + amount.length.toString().padStart(2, '0') + amount;

      const contents = [
        'TRAM NAM HANH PHUC NHE',
        'KHAI TRUONG PHAT TAI',
      ...
      const madon = 'madon' + timestamp.slice(-4) + randomSuffix;
      const fullContent = `${madon} ${randomContent}`;

      const contentTag = '62' + (fullContent.length + 4).toString().padStart(2, '0') + '08' + fullContent.length.toString().padStart(2, '0') + fullContent;
      const crcTag = '6304';
      const fullDataWithoutCRC = baseData + amountTag + contentTag + crcTag;
      ...
      const crc = crc16ccitt(fullDataWithoutCRC);
      const fullData = fullDataWithoutCRC + crc;

      const qrCodeUrl = await qr.toDataURL(fullData);

      // Thêm thông tin thanh toán vào đây
      const inforPayment = {
        bankName: selectedBank.bankName,
        bankNumber: selectedBank.bankNumber,
        bankProvide: selectedBank.bankProvide,
        amount: amount,
  ...
  createRequestManualBank: async (req, res) => {
    try {
      ...
      // thông báo nap tien qua telegram
      if (botTeleConfig.status) {
        ...
        const amount2 = Helper.numberWithCommas(Number(amountDeposit));

        teleBotSendMsg(chatTeleConfig.paymentBank, messageTeleConfig.paymentBank, {
          '{{time}}': time,
          '{{username}}': username,
          '{{name}}': name,
          '{{phone}}': phone,
          '{{email}}': email,
          '{{amount}}': amount2,
          '{{bankReceivesMoney}}': bankDeposit,
          '{{bankReceivesNumber}}': numberDeposit,
          '{{UserReceivesMoney}}': nameDeposit,
  ...
  createRequestWithdraw: async (req, res) => {
    try {
      ...
      const chatTeleConfig = JSON.parse(fs.readFileSync(process.env.PWD + "/src/configs/telegram/chatGroup.json", "utf8"));
      const messageTeleConfig = JSON.parse(fs.readFileSync(process.env.PWD + "/src/configs/telegram/message.json", "utf8"));
      const trans_id = Helper.getRandomInt(10000000, 99999999);

      // thông báo rut tien qua telegram
      if (botTeleConfig.status) {
        const time = moment().format("DD/MM/YYYY HH:mm:ss");
        const username = req.user.username;
        const name = req.user.name;
